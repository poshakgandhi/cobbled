...
def get_mass_function_dist_plot(source, fit_samples=None, user=None) -> str:
    """
    Generates a Plotly scatter plot showing Binary Mass Function f(m) vs Delta Chi^2 for accepted orbit samples (up to 50 max).
    ...
    if hasattr(fit_samples, 'get_orbit'):
        ...
        for i in range(num_samples):
            ...
            fm = 1.03606e-7 * P * (abs(K) ** 3) * ((1.0 - e ** 2) ** 1.5)
        ...
        for i in range(num_samples):
            ...
            v0 = s['v0']
            phi = s.get('phi', 0.0)
            fm = s.get('f_m', 1.03606e-7 * P * (abs(K) ** 3) * ((1.0 - e ** 2) ** 1.5))
    ...
    if num_samples > 1:
        fig.add_trace(go.Scatter(
            ...
            marker=dict(
                ...
                colorscale='Plasma',
                showscale=True,
                colorbar=dict(title='Period P (d)', thickness=12, len=0.8),
                line=dict(width=1, color='#1e293b'),
                opacity=0.85
            ),
            text=[f"Sample #{i+1}<br>f(m): {fm_vals[i]:.4f} M_☉<br>Δχ²: {deltachi2_vals[i]:.2f}<br>Period: {periods[i]:.2f} d<br>e: {ecc_vals[i]:.3f}" for i in range(1, num_samples)],
            hoverinfo='text'
        ))
    ...
    fig.add_trace(go.Scatter(
        ...
        text=[f"  MAP f(m) = {fm_vals[0]:.4f} M_☉"],
        ...
        hoverinfo='text',
        hovertext=[f"<b>MAP Solution (Best-Fit)</b><br>Mass Function f(m): {fm_vals[0]:.4f} M_☉<br>Δχ²: 0.00 (Minimum)<br>Period: {periods[0]:.4f} d<br>e: {ecc_vals[0]:.3f}"]
    ))

    # Add 1-sigma & 3-sigma confidence thresholds
    max_dchi2 = max(10.0, np.max(deltachi2_vals) * 1.1)
    min_fm = np.min(fm_vals) * 0.9
    max_fm = np.max(fm_vals) * 1.1
    ...
    fig.update_layout(
        title=dict(
            text=f"<b>Binary Mass Function f(m) vs. Δχ² Distribution</b> (Top {num_samples} Accepted Orbits)",
            font=dict(size=16)
        ),
        xaxis=dict(
            title="<b>Binary Mass Function f(m) (M<sub>☉</sub>)</b>",
            showgrid=True,
            gridcolor='#e2e8f0'
        ),
        yaxis=dict(
